...
%macro n_percentage(data_in=
 ...
 %local i
	...
	%if ^%library_exists(libref=avgml) %then %return;
	...
	%if &display_option %then %do;
		...
		%if ^%dataset_exists(dataset=avgml.percent) %then %do;
			...
			%return;
		%end;
		%if ^%variable_is_numeric(data_in=avgml.percent,varIn=&denom_var_in) %then %return;
	%end;
	...
	data &data_out;
		set %if &display_option %then avgml.percent;
			%else &data_in;;
		length &var_out $200;
